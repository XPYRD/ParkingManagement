  ...
  <div class="w-full h-full bg-white overflow-auto relative flex flex-col">
    ...
    <div class="bg-gray-100 border-b border-gray-200 px-4 py-3 flex items-center gap-2">
      ...
      <span class="ml-auto text-xs text-gray-500">
        传入停车位数: {{ props.spots.length }} | 过滤后: {{ filteredSpots.length }}
      </span>
    </div>

    <!-- 调试信息 -->
    <div class="bg-yellow-50 border-b border-yellow-200 px-4 py-2 text-xs">
      <div class="font-mono">
        <div>📊 第一个停车位数据: {{ filteredSpots[0] ? JSON.stringify({
          space_id: filteredSpots[0].space_id,
          x: filteredSpots[0].x,
          y: filteredSpots[0].y,
          center_x: filteredSpots[0].center_x,
          center_y: filteredSpots[0].center_y,
          status: filteredSpots[0].status
        }) : 'N/A' }}</div>
        <div>🎨 SVG ViewBox: {{ svgViewBox.width }} × {{ svgViewBox.height }}</div>
        <div>✅ SVG 已加载: {{ !!svgContent }}</div>
      </div>
    </div>
...
const filteredSpots = computed(() => {
  ...
  console.log('filteredSpots computed:', {
    allSpots: props.spots.length,
    floor: selectedFloor.value,
    filtered: props.spots.filter(spot => spot.floor === selectedFloor.value).length
  })
...
const getSpotX = (spot) => {
  ...
  if (spot.center_x !== undefined && spot.center_x !== null) {
    return parseFloat(spot.center_x) - 23.3
  }
...
onMounted(() => {
  console.log('ParkingMapSVG mounted, props.spots:', props.spots.length)
  loadSvg()
})
</script>
